  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <section className="mx-auto max-w-5xl px-6 py-10">
        ...
        <p className="mt-2 text-slate-300">Local preview. Backend API integration coming soon.</p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {choices.map((c) => (
            <button
              key={c}
              onClick={() => play(c)}
              className="rounded-xl border border-white/10 bg-white/5 px-6 py-4 text-lg transition hover:-translate-y-0.5 hover:shadow"
            >
              {c}
            </button>
          ))}
